...
public class BeaconPanel extends JPanel {
	...
	public void refresh() {
		try {
			...
			System.out.println("BeaconPanel.refresh()에서 예외 발생 : "
					+ e.getMessage());
		}
	}

	/**
	 * 비콘 추가 창을 띄움. "추가" 버튼에 의해 호출
	 */
	public void showAddBeacon() {
		...
		try {
			...
			System.out.println("BeaconPanel.showAddBeacon()에서 예외 발생 "
					+ e.getMessage());
		}
	...
	public void showModifyBeacon() {
		...
		try {
			...
			System.out.println("BeaconPanel.showModifyBeacon()에서 예외 발생 "
					+ e.getMessage());
		}
	...
	/**
	 * 확인 여부를 묻고 선택한 비콘을 삭제함. "삭제" 버튼에 의해 호출
	 */
	public void deleteBeacon() {
		...
		if (res == 0) {
			try {
				...
				System.out.println("BeaconPanel.deleteBeacon()에서 예외 발생 : "
						+ e.getMessage());
			}
		}
