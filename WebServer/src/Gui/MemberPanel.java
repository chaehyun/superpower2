...
public class MemberPanel extends JPanel {
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() % 2 == 0) {
					Personal personal = new Personal(rowDatas.get(
							table.getSelectedRow()).get(0));
	...
	public void refresh() {
		try {
			...
			System.out.println("MemberPanel.refresh()에서 예외 발생 : "
					+ e.getMessage());
		}
	}
	...
	public void showModifyMember() {
		...
		try {
			...
			System.out.println("MemberPanel.showModifyMember()에서 예외 발생 "
					+ e.getMessage());
		}
	...
	public void deleteMember() {
		...
		if (res == 0) {
			try {
				...
				// 삭제 수행
				DeleteMember.doAction(selectedId);

			} catch (SQLException e) {
				System.out.println("MemberPanel.deleteMember()에서 예외 발생 : "
						+ e.getMessage());
			}
		}
